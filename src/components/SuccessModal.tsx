  ...
  useEffect(() => {
    if (isOpen) {
      ...
      confetti({
        ...
        spread: 70,
        origin: { y: 0.6 },
        colors: ['#E86A33', '#F97316', '#10B981', '#3B82F6'],
      });
    }
  }, [isOpen]);
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/75 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-slate-100 overflow-hidden text-center p-6 sm:p-8 animate-in fade-in zoom-in duration-200">
        ...
        <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col gap-2">
          <button
            onClick={onReset}
            className="w-full py-3 bg-[#E86A33] hover:bg-[#D85820] text-white text-xs sm:text-sm font-bold rounded-xl shadow-md transition-all flex items-center justify-center gap-1.5 cursor-pointer"
          >
